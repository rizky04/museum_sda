...
<head>
    ...
    <style>
        ...
        .nav-link {
            padding: 18px 0; font-size: 13px; font-weight: 700; color: #78716c;
            letter-spacing: 0.05em; border-bottom: 3px solid transparent; transition: all 0.3s;
        }
        ...
        .manuskrip-card {
            ...
            transition: all 0.4s cubic-bezier(0.4, 0, 0.2, 1); border: 1px solid #f5ebe0;
        }
        ...
        .sub-nav-btn {
            padding: 10px 20px; background: #ffffff; border: 1px solid #e7e5e4;
            border-radius: 12px; font-size: 12px; font-weight: 700; color: #57534e;
            transition: all 0.2s;
        }

        .sub-nav-btn.active {
            background: #78350f; color: white; border-color: #78350f;
...
<body class="min-h-screen">
    ...
    {{-- <div id="bookModal" class="fixed inset-0 z-[60] hidden flex items-center justify-center p-4 md:p-10 bg-stone-900/95 backdrop-blur-md">
        <div class="bg-white w-full max-w-6xl h-full md:h-[85vh] flex flex-col md:flex-row shadow-2xl overflow-hidden md:rounded-[40px] animate-in zoom-in duration-300">

            <div class="w-full md:w-5/12 modal-gradient p-12 flex items-center justify-center relative border-r border-stone-100">
                <img id="modalCover" src="" class="max-h-full rounded-2xl shadow-2xl object-contain bg-white">
            </div>
            ...
            <div class="w-full md:w-7/12 flex flex-col bg-white">
                ...
                <div class="flex-grow overflow-y-auto p-10 pt-0 custom-scroll">
                    <div class="grid grid-cols-2 gap-4 mb-8">
                        <div class="bg-stone-50 p-4 rounded-2xl border border-stone-100">
                            <span class="text-[9px] font-extrabold text-stone-400 uppercase block mb-1">Kode Naskah</span>
                            <span id="modalCode" class="text-stone-900 font-bold"></span>
                        </div>
                        <div class="bg-stone-50 p-4 rounded-2xl border border-stone-100">
                            <span class="text-[9px] font-extrabold text-stone-400 uppercase block mb-1">Penulis</span>
                            <span id="modalAuthor" class="text-stone-900 font-bold"></span>
                        </div>
                    </div>
                    ...
                    <div class="border-t border-stone-100 pt-8">
                        <h4 class="text-xs font-extrabold text-amber-900 uppercase tracking-widest mb-4"><i class="fa-solid fa-comments mr-2"></i>Diskusi & Komentar</h4>

                        <div class="bg-stone-50 p-5 rounded-2xl mb-6 border border-stone-200">
                            <div class="grid grid-cols-2 gap-3 mb-3">
                                ...
                                <input type="text" id="commentPhone" placeholder="WhatsApp (Optional)" class="w-full bg-white border border-stone-200 rounded-xl px-4 py-2 text-sm outline-none focus:border-amber-500">
                            </div>
                            ...
                            <button onclick="submitComment()" class="w-full py-3 bg-amber-800 text-white text-xs font-bold rounded-xl hover:bg-amber-900 shadow-md transition-all">KIRIM KOMENTAR</button>
                        </div>
                ...
                <div class="p-8 border-t border-stone-100">
                    <button onclick="openPDF()" class="w-full py-4 bg-amber-700 text-white font-bold rounded-2xl hover:bg-amber-800 flex items-center justify-center gap-3">
    ...
    <div class="bg-white w-full max-w-6xl h-full md:h-[85vh] flex flex-col md:flex-row shadow-2xl overflow-y-auto md:overflow-hidden md:rounded-[40px] animate-in zoom-in duration-300">
        ...
        <div class="w-full md:w-5/12 modal-gradient p-6 md:p-12 flex items-center justify-center relative border-b md:border-b-0 md:border-r border-stone-100 min-h-[300px] md:min-h-0">
            <img id="modalCover" src="" class="h-64 md:h-auto md:max-h-full rounded-2xl shadow-2xl object-contain bg-white transform transition-transform duration-500 hover:scale-105">
        </div>
            ...
            <div class="flex-grow overflow-y-auto p-6 md:p-10 pt-0 custom-scroll">
                <div class="grid grid-cols-2 gap-3 md:gap-4 mb-6 md:mb-8">
                    <div class="bg-stone-50 p-3 md:p-4 rounded-2xl border border-stone-100">
                        ...
                        <span id="modalCode" class="text-stone-900 font-bold text-sm md:text-base"></span>
                    </div>
                    <div class="bg-stone-50 p-3 md:p-4 rounded-2xl border border-stone-100">
                        <span class="text-[8px] md:text-[9px] font-extrabold text-stone-400 uppercase block mb-1">Penulis</span>
                        <span id="modalAuthor" class="text-stone-900 font-bold text-sm md:text-base"></span>
                    </div>
                </div>
    ...
    <script>
        ...
        function displayBooks(data) {
            ...
            if (data.length === 0) {
                grid.innerHTML = `<div class="col-span-full py-32 text-center text-stone-400 font-bold uppercase">Tidak ditemukan</div>`;
                return;
            }
        ...
        async function fetchComments(bookId) {
            ...
            list.innerHTML = '<p class="text-stone-400 text-xs italic">Memuat diskusi...</p>';
            ...
            try {
                ...
                if (comments.length === 0) {
                    list.innerHTML = '<p class="text-stone-400 text-xs italic">Belum ada diskusi. Jadilah yang pertama!</p>';
                    return;
                }
                ...
                list.innerHTML = comments.map(c => `
                    <div class="bg-stone-50 p-4 rounded-xl border border-stone-100">
                        ...
                        <p class="text-stone-600 text-sm leading-relaxed">${c.comment}</p>
                    </div>
                `).join('');
            } catch (e) {
                list.innerHTML = '<p class="text-red-400 text-xs">Gagal memuat komentar.</p>';
            }
        }
        ...
        async function submitComment() {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    alert('Gagal mengirim: ' + JSON.stringify(err.errors));
                }
            } catch (e) {
        ...
        function handleParentClick(id, name) {
            ...
            if (children.length > 0) {
                ...
                document.getElementById('childNav').innerHTML = children.map(c => `
                    <button onclick="handleChildClick(${c.id}, '${c.category_name}')" class="sub-nav-btn" id="child-${c.id}">${c.category_name.toUpperCase()}</button>
                `).join('');
        ...
        function handleChildClick(id, name) {
            ...
            document.getElementById(`child-${id}`).classList.add('active');
        ...
        function filterByCategoryId(id) {
            const getIds = (parentId) => {
                let res = [parentId];
                allCategories.filter(c => c.parent_id === parentId).forEach(c => res = [...res, ...getIds(c.id)]);
                return res;
            };
            const targetIds = getIds(id);
            displayBooks(books.filter(b => targetIds.includes(b.category_id)));
        }
